...
extension IATBTPeripheral : CBCentralManagerDelegate {
    ...
    // MARK: - using responds so that class can optionally support state restoration
    // at the library users discression.
    // The fact that CoreBluetooth is checking if the willRestoreState method exists
    // so that it can attempt to be slightly more informative to us users really sucks.
    ...
    open func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData: [String : Any], rssi RSSI: NSNumber) {
        ...
        if isConnectable && self.peripheral == nil && peripheral.state == .disconnected {
